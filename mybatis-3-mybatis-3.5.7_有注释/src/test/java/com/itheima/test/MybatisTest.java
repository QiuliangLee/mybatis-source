...
public class MybatisTest {
  ...
  public void test1() throws IOException {
    ...
    User user = sqlSession.selectOne("com.itheima.mapper.UserMapper.findUserById", 1);
  ...
  @Test
  public void testMyMoney() {
    Double[] insurance = new Double[]{
            1.0, 8374.0, 21694.0, 40722.0, 69008.0, 100740.0, 105224.0, 109910.0, 113808.0, 119930.0, 125284.0,
            1.0, 1.0, 1.0, 1.0, 148622.0, 1.0, 1.0, 1.0, 1.0, 17638.0,
            1.0, 1.0, 1.0, 1.0, 20970.0, 1.0, 1.0, 1.0, 1.0, 248206.0,
            1.0, 1.0, 1.0, 1.0, 294306.0, 1.0, 1.0, 1.0, 1.0, 348808.0,
            1.0, 1.0, 1.0, 1.0, 413076.0, 1.0, 1.0, 1.0, 1.0, 488664.0,
            1.0, 1.0, 1.0, 1.0, 577430.0, 1.0, 1.0, 1.0, 1.0, 488664.0,
    };
    int increaseMoney = 20000;
    double totalMoney = 0L;
    double lastTotalMoney = 1L;
    for (int year = 1; ; year++) {
      if (year <= 5) {
        totalMoney += increaseMoney;
      }
      totalMoney += totalMoney * 0.0190;
      if (year % 5 == 0) {
        lastTotalMoney = totalMoney;
      }
//            System.out.println("第" + year + "年结束，我的保险盈利是" + (insurance[year] - totalMoney) / (insurance[year - 1] - totalMoney) * 100);
      if (insurance[year] >= totalMoney) {
        System.out.println("第" + year + "年结束，我的存款总额是" + totalMoney);
        System.out.println("我开始盈利了，盈利" + (insurance[year] - totalMoney) + "元,同比增长" + (insurance[year] - totalMoney) / (insurance[year - 5] - lastTotalMoney));
      }
      if (year == 51) {
        break;
      }
